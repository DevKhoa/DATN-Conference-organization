...
export const measureWrappedTextHeight = (
  ...
  if (ctx) {
    ...
    return Math.ceil(totalLines * fontPx * lineHeightMultiplier * 1.1);
  }
  ...
  const charsPerLine = Math.max(
    1,
    Math.floor(maxWidth / (fontPx * charWidthRatio)),
  );
  let totalLines = 0;
  for (const para of text.split("\n")) {
    if (!para.trim()) {
...
export const buildEditorPages = (data: any): EditorPage[] => {
  ...
  const addTMultiPage = (
    ...
    try {
      ...
      const charsPerLine = Math.max(1, Math.floor(w / (fontPx * 0.5)));
      const words = text.split(/\s+/);
      let cur = "";
      for (const word of words) {
        if (cur && (cur + " " + word).length > charsPerLine) {
          allLines.push(cur);
          cur = word;
  ...
  const addSecHeader = (title: string) => {
    const fsRaw = Math.max(
      10,
      Math.min(13, Math.floor(450 / (title.length * 0.55))),
    );
  ...
  // ── COVER ─────────────────────────────────────────────────────────────────
  {
    ...
    const titleFs = Math.max(
      ...
      Math.min(
        Math.round(25 * scY),
        Math.round((CW * 0.85) / (title.length * 0.55)),
      ),
    );
    ...
    if (data.cover?.conferenceName) {
      const nameH = measureWrappedTextHeight(
    ...
    if (dl) {
      const dlFs = Math.max(
        ...
        Math.min(
          Math.round(10 * scY),
          Math.round((CW * 0.9) / (dl.length * 0.52)),
        ),
      );
      addC({
        id: generateUUID(),
  ...
  const addInfoSection = (label: string, text?: string) => {
    ...
    const labelFs = Math.max(
      ...
      Math.min(
        Math.round(9 * scY),
        Math.round((CW * 0.9) / (label.length * 0.52)),
      ),
    );
    addTAt(
      label.toUpperCase(),
  ...
  if (Object.keys(byDate).length === 0) {
    ...
    Object.entries(byDate).forEach(([dateStr, items]) => {
      ...
      const dayFs = Math.max(
        ...
        Math.min(
          Math.round(12 * scY),
          Math.round((CW * 0.25) / (dayLabel.length * 0.6)),
        ),
      );
      const dateFs = Math.max(
        Math.round(8 * scY),
        Math.min(
          Math.round(12 * scY),
          Math.round((CW * 0.75) / (dateLabel.length * 0.55)),
        ),
      );
      hr.push({
        id: generateUUID(),
  ...
  if (data.keynotes?.length > 0) {
    (data.keynotes as any[]).forEach((k, idx) => {
      ...
      const pTitleFs = Math.max(
        9,
        Math.min(13, Math.floor(370 / (Math.max(1, pTitle.length) * 0.55))),
      );
      const _infoWForMeasure =
        CW - Math.round(130 * scX) - Math.round(30 * scX);
      const titleH_measured = measureWrappedTextHeight(
        pTitle,
        Math.round(pTitleFs * scY),
        _infoWForMeasure,
        1.3,
      );
  ...
  if (schedule.length === 0) {
    ...
    days.forEach((day) => {
      ...
      const dayLabelFs = Math.max(
        ...
        Math.min(
          Math.round(10 * scY),
          Math.round(((CW - 16 * scX) * 0.95) / (day.label.length * 0.55)),
        ),
      );
      addTAt(
        day.label,
  ...
  entries.forEach((entry) => {
    ...
    const labelFs = Math.max(
      ...
      Math.min(
        Math.round(10 * scY),
        Math.round(
          ((CW - numW - 75 * scX) * 0.95) / (entry.label.length * 0.55),
        ),
      ),
    );
    tocEls.push({
