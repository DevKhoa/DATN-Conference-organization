    ...
    <>
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        updateCover({
                          sponsorLogos: procData.cover.sponsorLogos.map(
                            (lg, i) =>
                              i === idx ? { ...lg, selected: checked } : lg,
                          ),
                        });
                      }}
                      className="absolute -top-2 -left-2 w-4 h-4 accent-indigo-600 rounded cursor-pointer z-10"
                      title={logo.selected ? "Deselect" : "Select to show"}
                    />
                    <button
                      onClick={() =>
                        updateCover({
                          sponsorLogos: procData.cover.sponsorLogos.filter(
                            (_, i) => i !== idx,
                          ),
                        })
                      }
      ...
      {activeTab === "committee" && (
        <div className="space-y-5">
          ...
          {(committeeActiveRole === "All"
            ? Object.keys(committeeByRole)
            : [committeeActiveRole].filter((r) => committeeByRole[r])
          ).map((role) => {
                ...
                {!isCollapsed && (
                  <div className="divide-y divide-slate-100">
                    {members.map((m: any, mIdx: number) => (
                      <div
                        key={m.id}
                        className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50/60 group"
                      >
                        ...
                        <input
                          ...
                          value={m.name}
                          onChange={(e) =>
                            patchCommitteeMember(m.id, { name: e.target.value })
                          }
                        />
                        <input
                        ...
                        <select
                          value={m.role}
                          onChange={(e) =>
                            patchCommitteeMember(m.id, { role: e.target.value })
                          }
        ...
        <div className="space-y-5 max-w-2xl">
          {[
            ...
            },
            {
              ...
              label: "Gala Dinner",
              rows: 2,
              placeholder: "Venue name, address, date, time, bus pickup…",
            },
      ...
      {activeTab === "schedule" && (
        <div className="space-y-4">
          ...
          <p className="text-xs text-slate-400">
            Session schedule is auto-loaded from the database. Edit sessions via
            the Sessions management screen.
          </p>
        </div>
      )}
      ...
      {activeTab === "keynotes" && (
        <div className="space-y-5">
          {procData.keynotes.length === 0 && (
            <div className="bg-slate-50 border border-dashed border-slate-300 rounded-xl p-8 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Keynote speakers will appear in the PDF after the schedule
                section.
              </p>
            </div>
          )}
          {procData.keynotes.map((k, idx) => {
            ...
            const isDuplicateSpeaker =
              k.name.trim() !== "" &&
              procData.keynotes.some(
                (other, otherIdx) =>
                  otherIdx !== idx &&
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center justify-between px-5 py-3 border-b ${isDuplicateSpeaker ? "bg-amber-50 border-amber-200" : "bg-slate-50 border-slate-200"}`}
                >
                  <div className="flex items-center gap-2">
                ...
                <div className="p-5 grid grid-cols-3 gap-5">
                  ...
                  <div className="col-span-2 space-y-3">
                    <div className="relative">
                      ...
                      />
                      {(getKState(k.id).userResults.length > 0 ||
                    ...
                    <div className="relative">
                      ...
                      <input
                        className={fieldCls}
                        value={
                          getKState(k.id).isEditingTitle
                            ? getKState(k.id).paperQuery
                            : k.presentationTitle
                        }
                        ...
                        onFocus={() => {
                          ...
                          patchKState(k.id, {
                            isEditingTitle: true,
                            paperQuery: k.presentationTitle,
                          });
                        }}
                        onBlur={() =>
                          setTimeout(() => {
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            <p className="text-xs text-slate-400">
              ...
              {Math.max(papersTotal, procData.detailedSchedule.length)} papers
              (status = ACCEPTED).
            </p>
            {papersError && (
            ...
            <p className="text-xs text-slate-400 flex items-center gap-1.5">
              ...
              {previewGenerating
                ...
                : previewBlobUrl
                  ...
                  : procData.detailedSchedule.length <
                      Math.max(papersTotal, procData.detailedSchedule.length)
          ...
          <div className="h-[720px] rounded-xl overflow-hidden border border-slate-200 bg-slate-100">
            {previewGenerating ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-4 bg-slate-50">
                ...
                <p className="text-sm text-slate-500">
                  {Math.max(papersTotal, procData.detailedSchedule.length)}{" "}
                  papers · {procData.keynotes.length} keynotes
                ...
                {edReady && (
                  <p className="text-xs text-slate-400 text-center max-w-xs">
                    Includes {edPages.length} pages from PDF Editor (with your
                    inserted pages)
                  </p>
                )}
              </div>
