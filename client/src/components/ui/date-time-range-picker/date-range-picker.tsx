...
/**
 * If toDate is the same day as fromDate, adjust it to current time
 * This ensures same-day searches capture data up to the current moment
 */
const adjustToDateIfSameDay = (fromDate: Date, toDate: Date): Date => {
  const isSameDay =
...
export const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-auto p-0" align={align} sideOffset={-200}>
          ...
          <div className="space-y-4">
              ...
              <Calendar
                ...
                onSelect={(newRange) => {
                  if (newRange && newRange.from && newRange.to) {
                    setRange({
                      from: newRange.from,
                      to: adjustToDateIfSameDay(newRange.from, newRange.to),
                    });
                  } else if (newRange) {
                ...
                disabled={(date) =>
                  disableFuture && date > new Date() ? true : false
                }
            ...
            <div className="flex justify-between items-center">
              <DateInput
                ...
                onChange={(date) => {
                  const toDate =
                    range.to === undefined || date > range.to ? date : range.to;
                  setRange((prevRange) => ({
                    ...prevRange,
                    from: date,
                    to: toDate,
