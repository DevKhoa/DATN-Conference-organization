...
const AssignSessionsPage: React.FC = () => {
  ...
  const handleSaveSessions = async () => {
    ...
    for (const s of sessions) {
      ...
      if (!sessionStart.isBefore(sessionEnd)) {
        setError(`Invalid time for session "${s.session_name}": Start time must be strictly before the end time.`);
        return;
      }

      // Check conference boundaries
      if (conferenceData?.conference?.start_date && conferenceData?.conference?.end_date) {
        const confStart = dayjs(conferenceData.conference.start_date).startOf("day");
      ...
      const parseTime = (t: string) => t.includes("T") ? dayjs(t) : dayjs(`2000-01-01T${t.length === 5 ? t + ':00' : t}`);
      ...
      for (let i = 0; i < s.assigned_papers.length; i++) {
        ...
        if (!p1Start.isBefore(p1End)) {
          setError(`In session "${s.session_name}", Paper #${i + 1} has an invalid time: Start time must be before end time.`);
          return;
        }
        ...
        for (let j = i + 1; j < s.assigned_papers.length; j++) {
          ...
          if (p1Start.isBefore(p2End) && p1End.isAfter(p2Start)) {
            setError(`In session "${s.session_name}", Paper #${i + 1} and Paper #${j + 1} have overlapping presentation times.`);
            return;
          }
        }
      }
  ...
  const handleRecommendChair = async (session: LocalSession) => {
    ...
    if (session.assigned_papers.length === 0) {
      setError(
        "This session has no papers. Cannot analyze context to suggest a Chair.",
      );
      return;
    }
            ...
            <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4">
              ...
              <div className="space-y-6">
                {sessions.map((session, idx) => (
                  <div
                    key={session.db_id}
                    id={`chair-session-${session.temp_id}`}
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <div className="relative max-w-md">
                        <label
                          htmlFor={`input-search-chair-${session.temp_id}`}
                        ...
                        <div className="relative group">
                          <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                          <input
                            id={`input-search-chair-${session.temp_id}`}
                            type="text"
                                ...
                                {availableChairs.filter((c) =>
                                  ...
                                  availableChairs
                                    ...
                                    .map((c) => (
                                      <div
                                        key={c.user_id}
                                        id={`dropdown-chair-${session.temp_id}-${c.user_id}`}
                        ...
                        {session.chair_person_id &&
                          chairSearchQueries[session.temp_id] === undefined && (
                            <div className="mt-3 inline-flex items-center gap-2 bg-indigo-50 border border-indigo-100 text-indigo-700 px-3 py-1.5 rounded-lg text-sm font-medium animate-in fade-in transition-all">
                              ...
                              <button
                                id={`btn-clear-selected-chair-${session.temp_id}`}
                    ...
                    <div className="md:w-[350px] border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-8 flex flex-col">
                      {!recommendations[session.temp_id] ? (
                        <div className="h-full flex flex-col items-center justify-center text-center p-6 bg-slate-50 rounded-xl border border-slate-100 border-dashed">
                          ...
                          <Button
                            id={`btn-recommend-chair-${session.temp_id}`}
                            variant="outline"
                        ...
                        <div className="space-y-3 animate-in fade-in">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <button
                              id={`btn-clear-recs-${session.temp_id}`}
                              onClick={() => {
                          ...
                          <div className="space-y-2">
                            {recommendations[session.temp_id].map((rec) => (
                              <div
                                key={rec.user_id}
                                id={`rec-chair-${session.temp_id}-${rec.user_id}`}
                                onClick={() =>
                                  updateSession(
                                    session.temp_id,
                                    "chair_person_id",
