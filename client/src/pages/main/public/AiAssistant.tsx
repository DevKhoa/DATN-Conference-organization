...
const AiAssistantPage: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="flex flex-col h-[calc(100vh-64px)] bg-background font-sans text-foreground">
        {/* Header */}
        <div className="bg-card border-b border-border px-6 py-4 flex items-center gap-3 shadow-sm z-10">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span
                className={`w-2 h-2 rounded-full ${isSessionReady ? "bg-primary" : sessionError ? "bg-destructive" : "bg-secondary animate-pulse"}`}
              ></span>
        ...
        <div className="grow overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          {isTyping && messages[messages.length - 1].role === "user" && (
            <div className="flex items-start gap-4">
              <div className="w-8 h-8 rounded-full bg-card border border-border text-primary shadow-sm flex items-center justify-center">
        ...
        <div className="bg-card border-t border-border p-4 sm:p-6">
          ...
          >
            <input
              ...
              className="grow bg-background border border-input text-foreground text-sm rounded-xl focus:ring-2 focus:ring-ring focus:border-ring block w-full p-4 outline-none disabled:opacity-60 disabled:cursor-not-allowed transition-all shadow-inner"
              disabled={!isSessionReady || isTyping}
            />
            <Button
              type="submit"
              disabled={!inputValue.trim() || !isSessionReady || isTyping}
              className={`h-13.5 px-6 rounded-xl transition-all ${
          ...
          {sessionError && (
            <div className="text-center mt-2 text-xs text-destructive flex items-center justify-center gap-1">
              <AlertCircle className="w-3 h-3" /> Failed to connect to AI
              service. Please refresh the page.
            </div>
          )}
        </div>
      </div>
