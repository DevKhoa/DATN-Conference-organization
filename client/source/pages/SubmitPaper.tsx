            ...
            <div className="flex items-center justify-between relative">
               ...
               {[1, 2, 3].map((s) => (
                 <div key={s} className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${
                   step >= s ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-400 border border-slate-200'
                 }`}>
                   {s}
                 </div>
               ))}
        ...
        {!success && step === 3 && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8 animate-in slide-in-from-right-4">
             ...
             <div className="space-y-6 max-w-md mx-auto">
                ...
                <Button onClick={handleUploadVersion} className="w-full" size="lg" disabled={loading || !file}>{loading ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : "Submit Paper"}</Button>
             </div>
          </div>
        )}
