...
export const createScriptSession = async (userId: string, sessionId: string): Promise<boolean> => {
  ...
  try {
    ...
    logger.log(success ? 'response' : 'error', 'Script Session Creation Result', { status: res.status, success });
    return success;
  } catch (error) {
...
export const deleteScriptSession = async (userId: string, sessionId: string): Promise<void> => {
  const url = `${API_BASE_URL}/apps/${APP_NAME}/users/${userId}/sessions/${sessionId}`;
...
export const sendScriptMessage = async (
  ...
  userId: string,
  sessionId: string,
  onUpdate: (update: { type: 'text' | 'tool'; data?: string; toolName?: string; toolArgs?: any }) => void
): Promise<void> => {
  const payload = {
    app_name: APP_NAME,
    user_id: userId,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.log('error', `Script Agent: HTTP Error ${response.status}`, { error: errorText });
      onUpdate({ type: 'text', data: `[ERROR] ${response.status}: ${errorText}` });
      return;
    }
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (content.parts) {
            for (const part of content.parts) {
              ...
              if (part.functionCall) {
                logger.log('info', 'Script Agent: Function Call detected', part.functionCall);
                onUpdate({ 
                  type: 'tool', 
          ...
          if (trimmed && !trimmed.startsWith('{') && !trimmed.startsWith('[')) {
             ...
             logger.log('warn', 'Script Agent: Skipped potential raw JSON chunk', rawData);
          }
        }
      }
    }
    ...
    onUpdate({ type: 'text', data: `[ERROR] Connection failed: ${error instanceof Error ? error.message : String(error)}` });
  }
};