...
import { 
  Search, Filter, Calendar, User, FileText, ChevronDown, 
  ChevronUp, Loader2, BookOpen, GraduationCap, X, UploadCloud
} from 'lucide-react';
...
const Papers: React.FC<PapersProps> = ({ onNavigateHome, onNavigateDetail, onNavigateSubmit, userRoleId = 0 }) => {
  // --- State ---
  ...
  const [sortBy, setSortBy] = useState<'DATE_DESC' | 'DATE_ASC' | 'TITLE_AZ' | 'DECISION'>('DATE_DESC');
  ...
  const fetchPapers = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        const uniqueConfs = Array.from(new Set(data.map((p: any) => p.conference?.conf_name).filter(Boolean))) as string[];
        const uniqueAuthors = Array.from(new Set(data.map((p: any) => p.author?.full_name).filter(Boolean))) as string[];
  ...
  const getStatusBadge = (status: string) => {
    ...
    switch (s) {
      case 'ACCEPTED':
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800 border border-green-200">Accepted</span>;
      case 'REJECTED':
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 border border-red-200">Rejected</span>;
      case 'UNDER_REVIEW':
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-200">Under Review</span>;
      case 'SUBMITTED':
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200">Submitted</span>;
      default:
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-600 border border-slate-200">{status}</span>;
    }
  };
  ...
  const filteredPapers = useMemo(() => {
    ...
    if (conferenceFilter !== 'ALL') {
      result = result.filter(p => p.conference?.conf_name === conferenceFilter);
    }
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'DECISION':
          const d1 = a.final_decision_date ? new Date(a.final_decision_date).getTime() : 0;
          const d2 = b.final_decision_date ? new Date(b.final_decision_date).getTime() : 0;
          return d2 - d1;
        case 'DATE_DESC':
        default:
    ...
    return result;
  }, [papers, searchTerm, statusFilter, conferenceFilter, authorFilter, sortBy]);
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="relative h-[250px] md:h-[300px] w-full overflow-hidden bg-slate-900 group">
        {HERO_IMAGES.map((img, idx) => (
          ...
          >
            <img src={img} alt="Archive Background" className="w-full h-full object-cover opacity-40" />
        ...
        <div className="absolute inset-0 flex flex-col justify-center items-center text-center px-4 z-10">
          <div className="bg-white/10 backdrop-blur-md border border-white/10 px-4 py-1.5 rounded-full mb-4">
            <span className="text-brand-300 font-bold tracking-wider text-xs uppercase">Knowledge Base</span>
          </div>
          ...
          <p className="text-slate-300 max-w-2xl text-lg font-light">
            Discover academic papers and research contributions shared by researchers from conferences around the world.
          </p>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm sticky top-24">
              ...
              <div className="mb-5">
                ...
                <div className="space-y-2">
                  {['ALL', 'SUBMITTED', 'UNDER_REVIEW', 'REJECTED', 'ACCEPTED'].map(status => (
                     <label key={status} className="flex items-center cursor-pointer group">
                        ...
                        />
                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center mr-2 transition-colors ${statusFilter === status ? 'border-brand-600 bg-brand-600' : 'border-slate-300 bg-white group-hover:border-brand-400'}`}>
                           {statusFilter === status && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                        </div>
                        <span className={`text-sm ${statusFilter === status ? 'text-brand-700 font-medium' : 'text-slate-600'}`}>
              ...
              {/* Conference Filter */}
              <div className="mb-5">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Conference</label>
              ...
              {/* Author Filter */}
              <div className="mb-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Primary Author</label>
                <div className="relative">
                   <select 
                      value={authorFilter}
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                <div className="py-20 text-center bg-white rounded-xl border border-red-100">
                  ...
                  <Button variant="ghost" onClick={fetchPapers} className="mt-2">Try Again</Button>
                </div>
                ...
                filteredPapers.map((paper) => (
                  <div 
                    ...
                    onClick={() => onNavigateDetail && onNavigateDetail(paper.paper_id)}
                  >
                    {/* Header Row */}
                      ...
                      <div className="flex-grow">
                         ...
                         <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-600 mb-3">
                            ...
                            <span className="flex items-center">
                               ...
                               <span className="italic">{paper.conference?.conf_name || 'Unknown Conference'}</span>
                            </span>
                         </div>
                      </div>
