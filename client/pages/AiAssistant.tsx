...
import { createScriptSession, sendScriptMessage, deleteScriptSession } from '../lib/ai-agent';
...
const AiAssistant: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'intro',
      role: 'assistant',
      content: "Hello! I'm your conference AI assistant. I can help you find information about papers, sessions, and schedules from our database. How can I assist you today?"
    }
  ]);
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!inputValue.trim() || !isSessionReady || isTyping || !sessionId || !userId) return;
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center gap-3 shadow-sm z-10">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isSessionReady ? 'bg-green-500' : sessionError ? 'bg-red-500' : 'bg-yellow-500 animate-pulse'}`}></span>
            <span className="text-xs text-slate-500">
              {isSessionReady ? 'Online' : sessionError ? 'Connection Failed' : 'Connecting...'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border-t border-slate-200 p-4 sm:p-6">
        <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto relative flex items-center gap-3">
          <input
            ...
            placeholder={isSessionReady ? "Ask about conferences, papers, or schedules..." : "Connecting to agent..."}
        ...
        {sessionError && (
          <div className="text-center mt-2 text-xs text-red-500 flex items-center justify-center gap-1">
            <AlertCircle className="w-3 h-3" /> Failed to connect to AI service. Please refresh the page.
          </div>
        )}
      </div>
    </div>
