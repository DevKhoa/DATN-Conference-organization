...
import { Mail, ArrowLeft, CheckCircle, Loader2, Lock, Globe, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
interface LoginProps {
  ...
  onLoginSuccess: (data: { name: string; email: string; role: string; roleId: number; avatar: string }) => void;
}

type LoginStep = 'email' | 'password' | 'success';

const Login: React.FC<LoginProps> = ({ onNavigateHome, onNavigateRegister, onLoginSuccess }) => {
  // --- STATE ---
  ...
  useEffect(() => {
    if (step === 'success' && userData) {
      ...
      const timer = setTimeout(() => {
        ...
        console.log('🔍 [Redirect] Inspecting user roles for final output:', userData.user_roles);
        ...
        if (userData.user_roles && userData.user_roles.length > 0) {
           ...
           if (userData.user_roles[0].roles) {
               ...
               console.log(`✅ [Redirect] Role detected: "${roleName}" (ID: ${roleId})`);
           }
        } else {
           console.warn('⚠️ [Redirect] No explicit role found. Defaulting to "User".');
        }
        ...
        console.log('🚀 [Redirect] Triggering onLoginSuccess with:', finalPayload);
          ...
          <div className="p-8">
            ...
            {step === 'email' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="text-center">
                  ...
                  <p className="text-slate-500 mt-2">Enter your email to sign in.</p>
                </div>
                ...
                <form onSubmit={handleEmailSubmit} className="space-y-6">
                  ...
                  <Button type="submit" className="w-full py-3 text-base shadow-lg shadow-brand-500/20" disabled={isLoading}>
                    {isLoading ? <Loader2 className="animate-spin w-5 h-5 mx-auto" /> : 'Continue'}
                  </Button>
                </form>
                ...
                <div className="text-center">
                  <p className="text-sm text-slate-500">
                    ...
                    <button onClick={onNavigateRegister} className="text-brand-600 hover:text-brand-700 font-semibold hover:underline">
                      Sign up
                    </button>
                  </p>
                </div>
            ...
            {step === 'password' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="text-center">
                  ...
                  <h2 className="text-xl font-bold text-slate-900">Hello, {userData?.full_name}</h2>
                  <button 
                    onClick={() => { setStep('email'); setEmail(''); setUserData(null); }}
            ...
            {step === 'success' && (
              <div className="text-center py-8 animate-in fade-in zoom-in-95 duration-500">
                  ...
                  <h2 className="text-2xl font-bold text-slate-900 mb-4">Welcome back, {userData?.full_name}</h2>
