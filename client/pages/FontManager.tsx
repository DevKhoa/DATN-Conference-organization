...
import { Search, Star, StarOff, ChevronDown, Type, X, Check } from 'lucide-react';
...
export const FONT_CATALOGUE: FontEntry[] = [
    ...
    { name: 'Libre Baskerville', category: 'serif', googleName: 'Libre+Baskerville' },
    ...
    { name: 'Cormorant Garamond', category: 'serif', googleName: 'Cormorant+Garamond' },
    ...
    { name: 'Source Sans 3', category: 'sans-serif', googleName: 'Source+Sans+3' },
    ...
    { name: 'Titillium Web', category: 'sans-serif', googleName: 'Titillium+Web' },
    ...
    { name: 'Source Code Pro', category: 'monospace', googleName: 'Source+Code+Pro' },
    ...
    { name: 'Noto Sans Mono', category: 'monospace', googleName: 'Noto+Sans+Mono' },
    ...
    { name: 'Permanent Marker', category: 'display', googleName: 'Permanent+Marker' },
    { name: 'Rubik', category: 'display', googleName: 'Rubik' },
    { name: 'Secular One', category: 'display', googleName: 'Secular+One' },
    { name: 'Yanone Kaffeesatz', category: 'display', googleName: 'Yanone+Kaffeesatz' },
    ...
    // ── Handwriting ───────────────────────────────────────────────────────────
    { name: 'Dancing Script', category: 'handwriting', googleName: 'Dancing+Script' },
    ...
    { name: 'Architects Daughter', category: 'handwriting', googleName: 'Architects+Daughter' },
...
export const loadFont = (entry: FontEntry) => {
    ...
    if (entry.googleName === 'Helvetica') { loadedFonts.add(entry.name); return; }
...
export const FontSelector: React.FC<FontSelectorProps> = ({ value, onChange, className = '' }) => {
    ...
    const [starred, setStarred] = useState<string[]>(() => {
        try { return JSON.parse(localStorage.getItem('fm_starred') || '[]'); } catch { return []; }
    });
    ...
    const categories = ['all', 'starred', 'serif', 'sans-serif', 'monospace', 'display', 'handwriting'];

    const filtered = FONT_CATALOGUE.filter(f => {
        const matchQ = !query || f.name.toLowerCase().includes(query.toLowerCase());
        const matchC = activeCategory === 'all' ? true
            : activeCategory === 'starred' ? starred.includes(f.name)
    ...
    return (
        <div ref={panelRef} className={`relative ${className}`}>
            ...
            >
                <span className="truncate text-slate-800">{value || 'Select font…'}</span>
                <ChevronDown className={`w-3.5 h-3.5 text-slate-400 shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
            </button>

            {/* Dropdown */}
            {open && (
                <div className="absolute z-50 top-full -left-4 sm:left-0 mt-1 w-full min-w-[240px] bg-white border border-slate-200 rounded-xl shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: 420 }}>
                    {/* Search */}
                    <div className="p-2 border-b border-slate-100 flex items-center gap-1.5">
                        <Search className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <input ref={inputRef} type="text" placeholder="Search fonts…" value={query}
                    ...
                    <div className="flex gap-0.5 px-2 pt-1.5 pb-1 overflow-x-auto shrink-0">
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setActiveCategory(cat)}
                                className={`px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap transition-all ${activeCategory === cat
                                    ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
                                {cat === 'starred' ? '★' : cat}
                            </button>
                        ))}
                    </div>
                            ...
                            return (
                                <div key={f.name} onClick={() => select(f)}
                                    ...
                                    <div className="flex items-center gap-1.5 shrink-0">
                                        ...
                                        <button onClick={e => toggleStar(f.name, e)}
...
const CATEGORIES = ['all', 'serif', 'sans-serif', 'monospace', 'display', 'handwriting'] as const;
...
export const FontManagerPage: React.FC<FontManagerPageProps> = ({ onBack }) => {
    ...
    const handleLoad = useCallback((f: FontEntry) => {
    ...
    useEffect(() => { filtered.forEach(f => handleLoad(f)); }, [filtered.map(f => f.name).join(',')]);

    const catCounts: Record<string, number> = { all: FONT_CATALOGUE.length };
    FONT_CATALOGUE.forEach(f => { catCounts[f.category] = (catCounts[f.category] || 0) + 1; });
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center gap-4 sticky top-0 z-10">
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">{FONT_CATALOGUE.length} fonts available</p>
                    </div>
                </div>
                ...
                <div className="ml-auto flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <input type="range" min={12} max={48} value={previewSize}
            ...
            <div className="flex flex-1 min-h-0">
                ...
                <div className="w-48 shrink-0 bg-white border-r border-slate-200 flex flex-col py-3 gap-0.5 px-2">
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider px-2 mb-1">Category</p>
                    {[...CATEGORIES, 'starred'].map(cat => (
                        <button key={cat} onClick={() => setActiveCategory(cat)}
                            className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all ${activeCategory === cat
                                ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
                            <span className="capitalize">{cat === 'starred' ? '★ Starred' : cat}</span>
                            <span className={`text-xs ${activeCategory === cat ? 'text-indigo-200' : 'text-slate-400'}`}>
                                {catCounts[cat] || 0}
                            </span>
                        </button>
                    ))}
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    ...
                    <div className="bg-white border-b border-slate-100 px-4 py-2.5 flex items-center gap-2">
                        ...
                        <input type="text" placeholder="Search fonts by name…" value={query}
                            onChange={e => setQuery(e.target.value)}
                            className="flex-1 text-sm outline-none text-slate-700 placeholder:text-slate-300 bg-transparent" />
                        {query && (
                            <button onClick={() => setQuery('')} className="p-0.5 hover:bg-slate-100 rounded">
                                <X className="w-3.5 h-3.5 text-slate-400" />
                            </button>
                        )}
                        <span className="text-xs text-slate-400">{filtered.length} fonts</span>
                    </div>
                        ...
                        {filtered.length === 0 ? (
                            ...
                            return (
                                <div key={f.name}
                                    ...
                                    {/* Star */}
                                    <button onClick={() => toggleStar(f.name)}
                                    ...
                                    <div className="w-44 shrink-0">
                                        ...
                                        <p className="text-[10px] text-slate-400 capitalize">{f.category}</p>
                                    </div>
                                    ...
                                    {/* Category badge */}
                                    <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium ${f.category === 'serif' ? 'bg-blue-50 text-blue-600'
                                        : f.category === 'sans-serif' ? 'bg-emerald-50 text-emerald-600'
                                            : f.category === 'monospace' ? 'bg-violet-50 text-violet-600'
                                                : f.category === 'display' ? 'bg-orange-50 text-orange-600'
