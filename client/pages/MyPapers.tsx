...
import { 
  FileText, Calendar, ChevronRight, Loader2, AlertCircle, 
...
const MyPapers: React.FC<MyPapersProps> = ({ userEmail, onNavigateDetail, onNavigateHome }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toUpperCase()) {
      case 'ACCEPTED':
        return <span className="flex items-center text-green-700 bg-green-50 px-2 py-1 rounded-md text-xs font-bold border border-green-200"><CheckCircle className="w-3 h-3 mr-1" /> Accepted</span>;
      case 'REJECTED':
        return <span className="flex items-center text-red-700 bg-red-50 px-2 py-1 rounded-md text-xs font-bold border border-red-200"><XCircle className="w-3 h-3 mr-1" /> Rejected</span>;
      case 'UNDER_REVIEW':
        return <span className="flex items-center text-yellow-700 bg-yellow-50 px-2 py-1 rounded-md text-xs font-bold border border-yellow-200"><Clock className="w-3 h-3 mr-1" /> Under Review</span>;
      default:
        return <span className="flex items-center text-blue-700 bg-blue-50 px-2 py-1 rounded-md text-xs font-bold border border-blue-200">{status}</span>;
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1">Manage and track your research submissions.</p>
          </div>
          <Button variant="ghost" onClick={onNavigateHome}>Dashboard</Button>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-slate-200 shadow-sm">
             ...
             <p className="text-slate-500 mt-1">You haven't submitted any papers yet.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-grow">
                    ...
                    <p className="text-sm text-slate-500 mb-3 line-clamp-2">{paper.abstract}</p>
                    <div className="text-xs font-medium text-slate-600 bg-slate-100 px-2 py-1 rounded inline-block">
                       Submitted to: {paper.conference?.conf_name || 'Unknown Conference'}
                    </div>
                  </div>
