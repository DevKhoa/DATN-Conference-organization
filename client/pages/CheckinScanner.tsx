...
const CheckinScanner: React.FC<CheckinScannerProps> = ({ onNavigateBack, sessionIds }) => {
    ...
    const getStatusIcon = () => {
        switch (scanResult.status) {
            case 'success': return <CheckCircle2 className="w-6 h-6 shrink-0 text-emerald-500" />;
            case 'error': return <XCircle className="w-6 h-6 shrink-0 text-red-500" />;
            case 'loading': return <Loader2 className="w-6 h-6 shrink-0 text-blue-500 animate-spin" />;
            default: return <RefreshCcw className="w-6 h-6 shrink-0 text-slate-400 animate-spin-slow" />;
        }
    };
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900 flex flex-col h-[100dvh]">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-4 overflow-y-auto">
                ...
                <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col">
                    ...
                    <div className="relative aspect-square sm:aspect-[4/5] w-full bg-slate-950 flex items-center justify-center overflow-hidden">
                        {hasCameraPermission === false && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-20 bg-slate-900 m-4 rounded-2xl border-2 border-slate-800">
                                ...
                                <p className="text-slate-400 text-xs sm:text-sm mb-4">Please allow camera permissions in your browser to scan QR codes.</p>
                                <Button onClick={() => window.location.reload()} className="w-full justify-center" variant="primary">
                                    Reload Page
                                </Button>
                            </div>
                        )}

                        <div id="qr-reader" className="w-full h-full [&>video]:object-cover" style={{ width: '100%', height: '100%', border: 'none' }}></div>
                    ...
                    {/* Scan Result Banner */}
                    <div className={`p-4 sm:p-5 flex items-start sm:items-center gap-3 transition-all duration-300 border-t ${getStatusColor()}`}>
                        <div className="shrink-0 bg-white p-2 sm:p-2.5 rounded-xl shadow-sm mt-0.5 sm:mt-0">
                            {getStatusIcon()}
                        </div>
                        <div className="flex-1 min-w-0">
                            <h3 className={`font-bold text-sm sm:text-base ${scanResult.status === 'success' ? 'text-emerald-900' : scanResult.status === 'error' ? 'text-red-900' : 'text-slate-900'}`}>
