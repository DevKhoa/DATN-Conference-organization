...
import {
    Plus, Trash2, Upload, Merge, SplitSquareHorizontal,
    AlignLeft, AlignCenter, AlignRight, X, Check, Grid3X3,
} from 'lucide-react';
...
export const createEmptyTable = (rows: number, cols: number, w: number, h: number): TableData => {
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            row.push({
                id: uuidv4(), text: '', align: 'center',
                colSpan: 1, rowSpan: 1, hidden: false,
                fontSize: 10, bold: false, italic: false, fontColor: '#1a202c',
            });
        }
        cells.push(row);
    }
    return {
        ...
        borderOn: true, borderThickness: 1, cellPadding: 4,
        headerHighlight: true, headerBgColor: '#1a3a6b', borderColor: '#cbd5e0',
    };
};
...
export const createTableFromCSV = (data: string[][], w: number): TableData => {
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            row.push({
                id: uuidv4(), text: data[r]?.[c] || '', align: 'left',
                colSpan: 1, rowSpan: 1, hidden: false,
                fontSize: 10, bold: false, italic: false, fontColor: '#1a202c',
            });
        }
...
const selBounds = (sel: CellCoord[]) => {
    ...
    let r1 = sel[0].r, c1 = sel[0].c, r2 = sel[0].r, c2 = sel[0].c;
    sel.forEach(({ r, c }) => { r1 = Math.min(r1, r); c1 = Math.min(c1, c); r2 = Math.max(r2, r); c2 = Math.max(c2, c); });
    return { r1, c1, r2, c2 };
};
...
export const mergeCells = (td: TableData, sel: CellCoord[]): TableData => {
    ...
    for (let r = b.r1; r <= b.r2; r++)
        for (let c = b.c1; c <= b.c2; c++) {
            ...
            nc[r][c] = { ...nc[r][c], hidden: true, text: '', colSpan: 1, rowSpan: 1 };
        }
...
export const splitSelected = (td: TableData, sel: CellCoord[]): TableData => {
    ...
    sel.forEach(({ r, c }) => {
        ...
        if (cell && (cell.colSpan > 1 || cell.rowSpan > 1)) res = splitCell(res, r, c);
    });
    return res;
};
...
export const addRow = (td: TableData, afterIdx?: number): TableData => {
    ...
    const newRow: TableCell[] = Array.from({ length: td.cols }, () => ({
        id: uuidv4(), text: '', align: 'left' as const, colSpan: 1, rowSpan: 1, hidden: false,
        fontSize: 10, bold: false, italic: false, fontColor: '#1a202c',
    }));
    const nc = [...td.cells]; nc.splice(idx, 0, newRow);
    const nh = [...td.rowHeights]; nh.splice(idx, 0, td.rowHeights[0] || 32);
...
export const deleteRow = (td: TableData, idx: number): TableData => {
    ...
    return { ...td, rows: td.rows - 1, cells: td.cells.filter((_, i) => i !== idx), rowHeights: td.rowHeights.filter((_, i) => i !== idx) };
};
export const addColumn = (td: TableData, afterIdx?: number): TableData => {
    ...
    const nc = td.cells.map(row => {
        const nr = [...row]; nr.splice(idx, 0, {
            id: uuidv4(), text: '', align: 'left' as const, colSpan: 1, rowSpan: 1, hidden: false,
            fontSize: 10, bold: false, italic: false, fontColor: '#1a202c',
        }); return nr;
    });
    const nw = [...td.colWidths]; nw.splice(idx, 0, td.colWidths[0] || 80);
...
export const deleteColumn = (td: TableData, idx: number): TableData => {
    ...
    return { ...td, cols: td.cols - 1, cells: td.cells.map(row => row.filter((_, i) => i !== idx)), colWidths: td.colWidths.filter((_, i) => i !== idx) };
};
...
export const TableEditorCanvas: React.FC<TableEditorCanvasProps> = ({
    tableData: td, elW, elH, isSelected, selectedCells, onSelectCells, onPatchTable,
}) => {
    ...
    const onColResizeDown = (e: React.PointerEvent, ci: number) => {
        ...
        setColResizeIdx(ci);
        resizeRef.current = { startX: e.clientX, startY: 0, origSize: td.colWidths[ci] };
    };
    const onRowResizeDown = (e: React.PointerEvent, ri: number) => {
        e.stopPropagation(); e.preventDefault();
        (e.currentTarget as Element).setPointerCapture(e.pointerId);
        setRowResizeIdx(ri);
        resizeRef.current = { startX: 0, startY: e.clientY, origSize: td.rowHeights[ri] };
    };
    const onResizeMove = (e: React.PointerEvent) => {
        if (colResizeIdx !== null) {
            const dx = e.clientX - resizeRef.current.startX;
            const nw = [...td.colWidths]; nw[colResizeIdx] = Math.max(30, resizeRef.current.origSize + dx / scX);
            onPatchTable({ ...td, colWidths: nw });
        }
        if (rowResizeIdx !== null) {
            const dy = e.clientY - resizeRef.current.startY;
            const nh = [...td.rowHeights]; nh[rowResizeIdx] = Math.max(20, resizeRef.current.origSize + dy / scY);
            onPatchTable({ ...td, rowHeights: nh });
        }
    };
    const onResizeUp = () => { setColResizeIdx(null); setRowResizeIdx(null); };
    ...
    const onCellClick = (e: React.MouseEvent, r: number, c: number) => {
        ...
        if (e.ctrlKey || e.metaKey) {
            ...
            if (isCellSel(selectedCells, r, c))
                ...
                onSelectCells([...selectedCells, { r, c }]);
        } else if (e.shiftKey && lastClickedRef.current) {
            // Range
            const lr = lastClickedRef.current.r, lc = lastClickedRef.current.c;
            const rMin = Math.min(lr, r), rMax = Math.max(lr, r);
            const cMin = Math.min(lc, c), cMax = Math.max(lc, c);
    ...
    const onRowHdrClick = (e: React.MouseEvent, r: number) => {
        ...
        for (let c = 0; c < td.cols; c++) if (!td.cells[r]?.[c]?.hidden) cells.push({ r, c });
        onSelectCells(e.ctrlKey ? [...selectedCells, ...cells.filter(nc2 => !isCellSel(selectedCells, nc2.r, nc2.c))] : cells);
    };
    const onColHdrClick = (e: React.MouseEvent, c: number) => {
        e.stopPropagation();
        const cells: CellCoord[] = [];
        for (let r = 0; r < td.rows; r++) if (!td.cells[r]?.[c]?.hidden) cells.push({ r, c });
        onSelectCells(e.ctrlKey ? [...selectedCells, ...cells.filter(nc2 => !isCellSel(selectedCells, nc2.r, nc2.c))] : cells);
    };
    ...
    return (
        ...
        >
            {/* ── Column headers ── */}
            {isSelected && scaledCW.map((w, ci) => (
                <div key={`ch-${ci}`} className="absolute select-none cursor-pointer hover:bg-indigo-100"
                    style={{
                        left: HDR_W + xPos[ci], top: 0, width: w, height: HDR_H,
                        fontSize: 8, color: '#6b7280', background: '#f1f5f9',
                        borderRight: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}
                    onClick={e => onColHdrClick(e, ci)}
                    onPointerDown={e => e.stopPropagation()}
                >{String.fromCharCode(65 + (ci % 26))}</div>
            ))}

            {/* ── Row headers ── */}
            {isSelected && scaledRH.map((h, ri) => (
                <div key={`rh-${ri}`} className="absolute select-none cursor-pointer hover:bg-indigo-100"
                    style={{
                        left: 0, top: HDR_H + yPos[ri], width: HDR_W, height: h,
                        fontSize: 8, color: '#6b7280', background: '#f1f5f9',
                        borderRight: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}
            ...
            {td.cells.map((row, ri) => row.map((cell, ci) => {
                ...
                const cw = (xPos[ci + cell.colSpan] ?? xPos[xPos.length - 1]) - xPos[ci];
                const ch = (yPos[ri + cell.rowSpan] ?? yPos[yPos.length - 1]) - yPos[ri];
                ...
                return (
                    <div key={cell.id}
                        ...
                        style={{
                            ...
                            backgroundColor: bg,
                            borderTopWidth: (cell.borderTop ?? td.borderOn) ? (cell.borderWidth ?? bw) : 0,
                            borderRightWidth: (cell.borderRight ?? td.borderOn) ? (cell.borderWidth ?? bw) : 0,
                            borderBottomWidth: (cell.borderBottom ?? td.borderOn) ? (cell.borderWidth ?? bw) : 0,
                            borderLeftWidth: (cell.borderLeft ?? td.borderOn) ? (cell.borderWidth ?? bw) : 0,
                            borderStyle: 'solid',
                            borderColor: cell.borderColor ?? td.borderColor,
                            padding: td.cellPadding, boxSizing: 'border-box', overflow: 'hidden',
                        ...
                        {isEdit ? (
                            <textarea
                                ...
                                style={{
                                    ...
                                    color: txtColor, textAlign: cell.align, lineHeight: 1.3,
                                    fontFamily: cell.fontFamily ? cssFontFamily(cell.fontFamily) : 'inherit',
                                }}
                                value={cell.text}
                                ...
                                onKeyDown={e => {
                                    ...
                                    if (e.key === 'Tab') {
                                        ...
                                        else if (ri + 1 < td.rows) setEditingCell({ r: ri + 1, c: 0 });
                                    }
                                }}
            ...
            {isSelected && scaledCW.map((_, ci) => {
                if (ci >= td.cols - 1) return null;
                return (
                    <div key={`cr-${ci}`} className="absolute"
                        style={{
                            left: HDR_W + xPos[ci + 1] - 3, top: HDR_H, width: 6,
                            height: yPos[yPos.length - 1], cursor: 'col-resize', zIndex: 20,
                            background: colResizeIdx === ci ? 'rgba(99,102,241,0.3)' : 'transparent',
                        }}
            ...
            {/* ── Row resize handles ── */}
            {isSelected && scaledRH.map((_, ri) => {
                if (ri >= td.rows - 1) return null;
                return (
                    <div key={`rr-${ri}`} className="absolute"
                        style={{
                            left: HDR_W, top: HDR_H + yPos[ri + 1] - 3,
                            width: xPos[xPos.length - 1], height: 6, cursor: 'row-resize', zIndex: 20,
                            background: rowResizeIdx === ri ? 'rgba(99,102,241,0.3)' : 'transparent',
                        }}
...
export const TablePropertiesPanel: React.FC<TablePropertiesPanelProps> = ({
    tableData: td, selectedCells, onPatchTable, elementW,
}) => {
    const L = "text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5";
    const B = "w-full py-1.5 text-xs border border-slate-200 rounded-lg hover:bg-slate-50 flex items-center justify-center gap-1.5 transition-all";
    ...
    const patchSel = (patch: Partial<TableCell>) => {
        const nc = td.cells.map(row => row.map(cl => ({ ...cl })));
        selectedCells.forEach(({ r, c }) => { if (nc[r]?.[c]) Object.assign(nc[r][c], patch); });
        onPatchTable({ ...td, cells: nc });
    };
    ...
    return (
        <div className="space-y-3">
            <div>
                ...
                <p className="text-xs text-slate-600">{td.rows}×{td.cols} · {selN} selected</p>
            </div>
            ...
            {hasSel && (
                <div className="border border-indigo-200 bg-indigo-50/50 rounded-lg p-3 space-y-3">
                    ...
                    {/* Font size & Family */}
                    <div>
                        <label className="text-[9px] text-slate-500 block mb-1">Font family & size</label>
                        <FontSelector 
                        ...
                        <input type="number" min={6} max={48}
                            ...
                            onChange={e => patchSel({ fontSize: Number(e.target.value) || 10 })}
                            className="w-full px-2 py-1 border border-slate-200 rounded text-xs outline-none focus:ring-1 focus:ring-indigo-400" />
                    </div>

                    {/* Bold / Italic */}
                    <div className="flex gap-1.5">
                        <button onClick={() => patchSel({ bold: sBold === true ? false : true })}
                            className={`flex-1 py-1.5 rounded-lg border text-xs font-bold transition-all ${sBold === true
                                ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'}`}>B</button>
                        <button onClick={() => patchSel({ italic: sItalic === true ? false : true })}
                            className={`flex-1 py-1.5 rounded-lg border text-xs italic transition-all ${sItalic === true
                                ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'}`}>I</button>
                    </div>
                    ...
                    <div className="flex gap-1.5">
                        {(['left', 'center', 'right'] as const).map(a => (
                            <button key={a} onClick={() => patchSel({ align: a })}
                                className={`flex-1 py-1.5 rounded-lg border flex items-center justify-center transition-all ${sAlign === a
                                    ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <input type="color" value={sFontColor === 'mixed' ? '#000000' : (sFontColor || '#1a202c')}
                            onChange={e => patchSel({ fontColor: e.target.value })}
                            className="w-6 h-6 rounded border-0 p-0 bg-transparent cursor-pointer" />
                        <span className="text-[9px] text-slate-500 font-mono">{sFontColor === 'mixed' ? 'Mixed' : (sFontColor || '#1a202c')}</span>
                    </div>

                    {/* BG color */}
                    <div className="flex items-center gap-2">
                        <span className="text-[9px] text-slate-500 w-10">BG</span>
                        <input type="color" value={sBgColor === 'mixed' ? '#ffffff' : (sBgColor || '#ffffff')}
                            onChange={e => patchSel({ bgColor: e.target.value })}
                            className="w-6 h-6 rounded border-0 p-0 bg-transparent cursor-pointer" />
                        <button onClick={() => patchSel({ bgColor: undefined })}
                            className="text-[9px] text-slate-400 hover:text-red-500 underline">Clear</button>
                    </div>

                    {/* Per-cell borders */}
                    <div>
                        <label className="text-[9px] text-slate-500 block mb-1">Cell Borders</label>
                        <div className="grid grid-cols-4 gap-1">
                            {(['borderTop','borderRight','borderBottom','borderLeft'] as const).map(side => {
                                ...
                                return (
                                    <button key={side}
                                        onClick={() => patchSel({ [side]: isOn ? false : true } as any)}
                                        className={`py-1 rounded text-[8px] border transition-all ${
                                            isOn ? 'bg-slate-700 text-white border-slate-700' : 'bg-white text-slate-400 border-slate-200 hover:bg-slate-50'
                                        }`}>{label[0].toUpperCase()}</button>
                                );
                            })}
                        </div>
                        <div className="flex items-center gap-2 mt-1.5">
                            <span className="text-[8px] text-slate-400">Color</span>
                            <input type="color" value={(getP('borderColor') as string) || td.borderColor}
                            ...
                            <input type="number" min={0} max={5} step={0.5}
                                ...
                                onChange={e => patchSel({ borderWidth: Number(e.target.value) })}
            ...
            <div className="border-t border-slate-100 pt-3">
                ...
                <div className="flex gap-1.5">
                    <button onClick={() => onPatchTable(addRow(td))} className={B}><Plus className="w-3 h-3" /> Add</button>
                    <button onClick={() => onPatchTable(deleteRow(td, td.rows - 1))}
            ...
            <div>
                ...
                <div className="flex gap-1.5">
                    <button onClick={() => onPatchTable(addColumn(td))} className={B}><Plus className="w-3 h-3" /> Add</button>
                    <button onClick={() => onPatchTable(deleteColumn(td, td.cols - 1))}
            ...
            <div className="border-t border-slate-100 pt-3">
                ...
                <label className="flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer mb-2">
                    <input type="checkbox" checked={td.borderOn}
                        onChange={e => onPatchTable({ ...td, borderOn: e.target.checked })}
                        className="accent-indigo-600 w-3.5 h-3.5" /> Show
                </label>
                {td.borderOn && (
                    <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                            <span className="text-[9px] text-slate-400 w-14">Thick</span>
                            <input type="range" min={0.5} max={4} step={0.5} value={td.borderThickness}
                                onChange={e => onPatchTable({ ...td, borderThickness: Number(e.target.value) })}
                                className="flex-1 accent-indigo-600" />
                            <span className="text-[9px] text-slate-500 w-6">{td.borderThickness}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-[9px] text-slate-400 w-14">Color</span>
                            <input type="color" value={td.borderColor}
                                onChange={e => onPatchTable({ ...td, borderColor: e.target.value })}
                                className="w-5 h-5 rounded border-0 p-0 bg-transparent cursor-pointer" />
                        </div>
                    </div>
                )}
            ...
            <div>
                ...
                <div className="flex items-center gap-2">
                    <input type="range" min={0} max={20} value={td.cellPadding}
                        onChange={e => onPatchTable({ ...td, cellPadding: Number(e.target.value) })}
                        className="flex-1 accent-indigo-600" />
                    <span className="text-[9px] text-slate-500 w-6">{td.cellPadding}</span>
                </div>
            </div>
            ...
            <div className="border-t border-slate-100 pt-3">
                ...
                <label className="flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer mb-2">
                    <input type="checkbox" checked={td.headerHighlight}
                        onChange={e => onPatchTable({ ...td, headerHighlight: e.target.checked })}
                        className="accent-indigo-600 w-3.5 h-3.5" /> Highlight
                </label>
                {td.headerHighlight && (
                    <div className="flex items-center gap-2">
                        <span className="text-[9px] text-slate-400">BG</span>
                        <input type="color" value={td.headerBgColor}
                            onChange={e => onPatchTable({ ...td, headerBgColor: e.target.value })}
                            className="w-5 h-5 rounded border-0 p-0 bg-transparent cursor-pointer" />
                    </div>
                )}
            </div>
...
export const InsertTableModal: React.FC<InsertTableModalProps> = ({ onInsert, onClose }) => {
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-[380px]" onClick={e => e.stopPropagation()}>
                ...
                <div className="mb-4">
                    ...
                    <div className="inline-grid gap-[2px] p-2 bg-slate-50 rounded-lg border border-slate-200"
                        ...
                        {Array.from({ length: MR * MC }).map((_, i) => {
                            ...
                            const active = r <= (hoverR >= 0 ? hoverR : rows - 1) && c <= (hoverC >= 0 ? hoverC : cols - 1);
                            return (
                                <div key={i}
                                    ...
                                    onClick={() => { setRows(r + 1); setCols(c + 1); setHoverR(-1); setHoverC(-1); }} />
                            );
                        })}
                    </div>
                    <p className="text-xs text-slate-500 mt-1.5">
                        {hoverR >= 0 ? `${hoverR + 1} × ${hoverC + 1}` : `${rows} × ${cols}`}
                    </p>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-5">
                    <div>
                        <label className="text-[10px] font-semibold text-slate-400 uppercase block mb-1">Rows</label>
                    ...
                    <div>
                        <label className="text-[10px] font-semibold text-slate-400 uppercase block mb-1">Columns</label>
...
export const renderTableToCanvas = (
    ctx: CanvasRenderingContext2D, td: TableData,
    x: number, y: number, w: number, h: number, thumbScale: number,
) => {
    ...
    for (let ri = 0; ri < td.rows; ri++) {
        for (let ci = 0; ci < td.cols; ci++) {
            ...
            const cx = x + xP[ci] * thumbScale, cy = y + yP[ri] * thumbScale;
            ...
            if (bw > 0) { ctx.strokeStyle = td.borderColor; ctx.lineWidth = bw; ctx.strokeRect(cx, cy, cw, ch); }
            if (cell.text) {
                ...
                if (cell.align === 'center') { const tw = ctx.measureText(txt).width; ctx.fillText(txt, cx + (cw - tw) / 2, cy + pad, maxW); }
                else if (cell.align === 'right') { const tw = ctx.measureText(txt).width; ctx.fillText(txt, cx + cw - tw - pad, cy + pad, maxW); }
