...
const AssignSessions: React.FC<AssignSessionsProps> = ({ conferenceId, userRoleId, onNavigateBack, initialSessionId }) => {
  ...
  const [expandedSessions, setExpandedSessions] = useState<Record<string, boolean>>({}); // Full view vs Partial (show 5)
  const [hiddenPresentations, setHiddenPresentations] = useState<Record<string, boolean>>({}); // Totally hidden state
  ...
  const handleSaveSessions = async () => {
    ...
    const formatPaperTime = (timeStr: string, sessionLocalStart: string) => {
      ...
      if (timeParsed.isValid() && sessionLocalStart) {
        ...
        return dayjs(`${baseDate} ${timeParsed.format('HH:mm:ss')}`).format('YYYY-MM-DD HH:mm:ss');
      }
      return timeStr;
    };
      ...
      for (const s of sessions) {
        ...
        if (s.assigned_papers.length > 0) {
          const paperInserts = s.assigned_papers.map((p, idx) => ({
            ...
            start_time: p.start_time ? formatPaperTime(p.start_time, localStart) : null,
            end_time: p.end_time ? formatPaperTime(p.end_time, localStart) : null
          }));
                ...
                <div className="space-y-6">
                  {sessions.map((session, idx) => (
                    ...
                    >
                      ...
                      <div className={`transition-all duration-300 ${dragOverSessionId === session.temp_id ? 'bg-indigo-50/20' : ''}`}>
                        <div className="px-6 py-4 flex items-center justify-between border-t border-slate-100 bg-slate-50/30">
                          <div className="flex items-center gap-2">
                            <Layers className="w-4 h-4 text-slate-400" />
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Presentations ({session.assigned_papers.length})</p>
                          </div>
                          ...
                          <div className="p-6 pt-2">
                            {session.assigned_papers.length === 0 ? (
                              <div className="py-10 border-2 border-dashed border-slate-200 rounded-2xl text-center bg-white/50 group/drop">
                                ...
                                <p className="text-sm text-slate-400 font-medium px-4">Drag & drop papers from the sidebar to assign them to this session</p>
                              </div>
                            ) : (
                              <div className="space-y-3">
                                {(() => {
                                  const isExpanded = expandedSessions[session.temp_id] || session.assigned_papers.length <= 5;
                                    ...
                                    <>
                                            ...
                                            <div className="flex items-start gap-4 overflow-hidden cursor-grab active:cursor-grabbing w-full lg:w-auto flex-grow group">
                                              ...
                                              <div className="flex-grow min-w-0">
                                                <div className="flex items-center gap-2 mb-1">
                                                  <span className="text-indigo-600 font-bold text-xs uppercase bg-indigo-50 px-2 py-0.5 rounded-full">Paper #{pIdx + 1}</span>
                                                  <span className="text-[10px] font-medium text-slate-400">ID: {p.paper_id}</span>
                                                </div>
                                                <h4 className="font-semibold text-slate-800 text-sm line-clamp-2 pr-2 leading-snug">{p.title}</h4>
                                                {p.author_name && <p className="text-[11px] text-slate-500 mt-1 font-medium">By: {p.author_name}</p>}
                                              </div>
                                            </div>
                                      ...
                                      {session.assigned_papers.length > 5 && (
                                        <div className="pt-2 flex justify-center">
                                          <button
                                            onClick={() => toggleExpand(session.temp_id)}
                                          ...
                                          >
                                            {expandedSessions[session.temp_id] ? (
                                              ...
                                              <>Show {session.assigned_papers.length - 5} More Papers <ChevronDown className="w-4 h-4 group-hover:translate-y-0.5 transition-transform" /></>
                                            )}
                                          </button>
                                        </div>
                                      )}
