...
const SubmitPaper: React.FC<SubmitPaperProps> = ({
  ...
  const handleUploadVersion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_API_URL}/papers/${selectedPaperId}/${versionData.version_id}/upload`,
  ...
  const filteredConferences = conferences.filter(
    (c) =>
      ...
      (c.location &&
        c.location.toLowerCase().includes(confSearch.toLowerCase())),
  );
                      ...
                      {existingPapers
                        ...
                        .map((paper) => (
                          ...
                          >
                            ...
                            <div className="text-xs text-slate-400">
                              Author: {paper.author?.full_name}
                            </div>
                          </div>
                        ))}
                      {existingPapers.filter((p) =>
                        p.title
                          .toLowerCase()
                          .includes(paperSearch.toLowerCase()),
                      ).length === 0 && (
