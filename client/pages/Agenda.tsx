...
import { 
  format, parseISO, differenceInMinutes, addDays, subDays, isToday, 
...
function computeColumnLayout(sessions: Session[]): Map<number, { col: number; totalCols: number }> {
  ...
  for (const session of sorted) {
    ...
    for (const cluster of clusters) {
      if (cluster.some(c => s < parseISO(c.end_time) && e > parseISO(c.start_time))) {
...
export default function MyAgenda({ onNavigateConferenceDetail }: MyAgendaProps) {
  ...
  const columnLayout = useMemo(() => computeColumnLayout(currentDaySessions), [currentDaySessions]);
  const hoursArray = Array.from({ length: 24 }, (_, i) => i);
  const getEventStyle = (startTime: string, endTime: string, col: number, totalCols: number) => {
  ...
  // --- Render ---
  if (loading) return <div className="p-8 flex justify-center text-gray-500">Loading agenda...</div>;
  if (error) return <div className="p-8 flex justify-center text-red-500 items-center gap-2"><AlertCircle size={20} /> {error}</div>;

  return (
    <div className={`mx-auto ${viewMode === 'timeline' ? 'max-w-7xl px-4 py-4 md:px-6 md:py-5 flex flex-col' : 'max-w-4xl p-6'} bg-gray-50 md:bg-white md:rounded-xl shadow-sm min-h-screen transition-all duration-300`}>
      
      {/* --- HEADER --- */}
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${viewMode === 'timeline' ? 'mb-4' : 'mb-8 border-b pb-4'}`}>
        <div>
                  ...
                  {daySessions.map((session) => (
                    <div key={session.session_id} className="relative pl-6">
                      ...
                      <div
                        ...
                        onClick={() => onNavigateConferenceDetail?.(session.conf_id)}
                      >
        ...
        <div className="flex-1 flex flex-col min-h-[600px]">
          ...
          <div className="bg-white rounded-t-xl border border-gray-200 shadow-sm px-4 py-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handlePrevDay} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors text-gray-500 border border-gray-200">
                <ChevronLeft size={16} />
              </button>
              <button onClick={handleNextDay} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors text-gray-500 border border-gray-200">
                <ChevronRight size={16} />
              </button>

              <div className={`flex items-center gap-2.5 px-3 py-1.5 rounded-xl ml-1
                ...
                <span className={`text-3xl font-bold leading-none tabular-nums ${isCurrentDay ? 'text-blue-600' : selectedDayCount > 0 ? 'text-emerald-600' : 'text-gray-800'}`}>
                  {format(selectedDate, 'dd')}
                </span>
                <div className="flex flex-col leading-tight">
                  <span className={`text-[10px] font-semibold uppercase tracking-widest ${isCurrentDay ? 'text-blue-400' : selectedDayCount > 0 ? 'text-emerald-400' : 'text-gray-400'}`}>
                ...
                <div className="flex flex-col gap-0.5 ml-1">
                  {isCurrentDay && <span className="text-[10px] font-semibold bg-blue-600 text-white px-2 py-0.5 rounded-full tracking-wide text-center">TODAY</span>}
                  {selectedDayCount > 0 && (
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full tracking-wide text-center ${isCurrentDay ? 'bg-blue-100 text-blue-700' : 'bg-emerald-500 text-white'}`}>
                      {selectedDayCount} session{selectedDayCount > 1 ? 's' : ''}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-2">
              {!isCurrentDay && (
                <button onClick={handleGoToday} className="text-xs font-semibold text-blue-600 hover:bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-lg transition-colors">
                  Today
                </button>
              )}
              <div className="relative" ref={calendarRef}>
                <button type="button" onClick={() => setShowCalendar(prev => !prev)} className={`flex items-center gap-1.5 text-xs font-medium cursor-pointer transition-colors px-3 py-1.5 rounded-lg border bg-white select-none ${showCalendar ? 'text-blue-600 border-blue-400 bg-blue-50' : 'text-gray-600 border-gray-200 hover:text-blue-600 hover:bg-blue-50'}`}>
                  ...
                  return (
                    <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-gray-200 z-50 p-3 w-60">
                      <div className="flex items-center justify-between mb-2 px-0.5">
                        <button type="button" onClick={() => setCalendarMonth(prev => subMonths(prev, 1))} className="p-1 rounded-md hover:bg-gray-100 text-gray-500"><ChevronLeft size={14} /></button>
                        <span className="text-xs font-semibold text-gray-700">{format(calendarMonth, 'MMMM yyyy')}</span>
                        <button type="button" onClick={() => setCalendarMonth(prev => addMonths(prev, 1))} className="p-1 rounded-md hover:bg-gray-100 text-gray-500"><ChevronRight size={14} /></button>
                      </div>
                      <div className="grid grid-cols-7 mb-1">
                        {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d => <div key={d} className="text-center text-[10px] font-semibold text-gray-400 py-0.5">{d}</div>)}
                      </div>
                      <div className="grid grid-cols-7 gap-y-0.5">
                        {cells.map((day, i) => {
                          ...
                          return (
                            <button key={ds} type="button" onClick={() => { setSelectedDate(day); setShowCalendar(false); }} className={`relative flex flex-col items-center justify-center rounded-md min-h-[30px] text-[11px] font-medium transition-colors ${!inMonth ? 'opacity-30 pointer-events-none' : ''} ${isSelDay ? 'bg-blue-600 text-white' : isTodayDay ? 'ring-1 ring-blue-400 text-blue-600 bg-blue-50' : count > 0 ? 'hover:bg-emerald-50 text-gray-700' : 'hover:bg-gray-100 text-gray-700'}`}>
                              <span className="leading-none">{day.getDate()}</span>
                              {count > 0 && !isSelDay && <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-emerald-400" />}
                            </button>
                          );
                        })}
                      </div>
          ...
          {/* TIMELINE GRID */}
          <div ref={timelineRef} className="bg-white rounded-b-xl border-x border-b border-gray-200 shadow-sm relative overflow-y-auto overflow-x-hidden flex-1">
            <div className="relative" style={{ height: `${24 * HOUR_HEIGHT}px`, minWidth: '480px' }}>
              <div className="absolute left-14 right-0 top-0 bottom-0 pointer-events-none">
                {hoursArray.map((hour) => (
                  <div key={hour} style={{ height: `${HOUR_HEIGHT}px` }} className={`border-t w-full ${hour % 2 === 0 ? 'border-gray-200' : 'border-gray-100/60'}`} />
                ))}
              </div>
              ...
              <div className="absolute left-14 right-2 top-0 bottom-0">
                ...
                {currentDaySessions.map((session) => {
                  const layout = columnLayout.get(session.session_id) ?? { col: 0, totalCols: 1 };
                  return (
                  <div key={session.session_id} className="absolute overflow-hidden cursor-pointer group z-20 rounded-md border-l-[3px] border-l-blue-500 border border-blue-100 bg-blue-50/90 hover:bg-blue-100 transition-colors shadow-sm px-2 py-1" style={getEventStyle(session.start_time, session.end_time, layout.col, layout.totalCols)} onClick={() => onNavigateConferenceDetail?.(session.conf_id)}>
                    ...
                    <div className="flex flex-wrap gap-x-2.5 gap-y-0 text-[10px] text-gray-500 mt-0.5 leading-tight">
                      <span className="flex items-center gap-0.5">
                        ...
                        <span className="truncate max-w-[120px]">{session.room_location || 'TBD'}</span>
                      </span>
                      <span className="flex items-center gap-0.5">
                        <User size={8} className="flex-shrink-0" />
                        <span className="truncate max-w-[120px]">{session.chair_name}</span>
                      </span>
                    </div>
                  </div>
                  );
