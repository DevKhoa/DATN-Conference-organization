...
import { 
  ArrowLeft, Calendar, User, FileText, Download, 
  MessageSquare, Star, CheckCircle, AlertCircle, 
  MapPin, Building, Share2, Eye
} from 'lucide-react';
...
const PaperDetail: React.FC<PaperDetailProps> = ({ paperId, onNavigateBack }) => {
    ...
    try {
      ...
      const { data: paperData, error: paperError } = await supabase
        ...
        .select(`
          ...
          author:users!primary_author_id (full_name, email, organization, description),
  ...
  const getRecommendationBadge = (rec: string) => {
    switch (rec) {
      case 'ACCEPT': return <span className="text-green-600 font-bold text-xs bg-green-50 px-2 py-1 rounded border border-green-100">ACCEPT</span>;
      case 'WEAK_ACCEPT': return <span className="text-emerald-600 font-bold text-xs bg-emerald-50 px-2 py-1 rounded border border-emerald-100">WEAK ACCEPT</span>;
      case 'REVISION': return <span className="text-yellow-600 font-bold text-xs bg-yellow-50 px-2 py-1 rounded border border-yellow-100">REVISION</span>;
      case 'REJECT': return <span className="text-red-600 font-bold text-xs bg-red-50 px-2 py-1 rounded border border-red-100">REJECT</span>;
      default: return <span className="text-slate-500 font-bold text-xs">{rec}</span>;
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 text-center max-w-md">
          ...
          <Button onClick={onNavigateBack}>Return to Archive</Button>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div className="flex-grow">
              <div className="flex flex-wrap items-center gap-3 mb-3">
                 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide border ${getStatusColor(paper.status)}`}>
                    {paper.status.replace('_', ' ')}
                 </span>
                 <span className="text-sm text-slate-400 flex items-center">
                    <Calendar className="w-3.5 h-3.5 mr-1" /> {formatDate(paper.created_at)}
                 </span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 {pdfUrl ? (
                   ...
                   >
                     <p className="text-slate-500">
                        ...
                        <a href={pdfUrl} className="text-brand-600 underline ml-1">Download the PDF</a>.
                     </p>
                   </iframe>
                 ) : (
                   <div className="text-center p-8">
                      ...
                      <h4 className="text-slate-900 font-medium mb-1">Preview Unavailable</h4>
                      <p className="text-slate-500 text-sm">This paper does not have a displayable version yet.</p>
                   </div>
                 )}
               </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              {reviews.length === 0 ? (
                <div className="text-center py-8 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                   <p className="text-slate-500 italic">No reviews published for this paper yet.</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {reviews.map((review) => (
                    <div key={review.review_id} className="border-b border-slate-100 last:border-0 pb-6 last:pb-0">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2">
                           ...
                           <div>
                              <p className="text-sm font-bold text-slate-900">{review.reviewer?.full_name || 'Anonymous Reviewer'}</p>
                              <p className="text-xs text-slate-400">{formatDate(review.review_date)}</p>
                           </div>
                        </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-24">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-4">Primary Author</h3>
              ...
              <div className="text-xs text-slate-400 pt-4 border-t border-slate-100 flex items-center">
                 <CheckCircle className="w-3 h-3 mr-1.5 text-green-500" /> Verified Researcher
              </div>
            </div>

            {/* 3. CONFERENCE INFO */}
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl shadow-lg p-6 text-white">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-4">Submitted To</h3>
              ...
              <div className="space-y-3 text-sm">
                 <div className="flex items-center text-slate-300">
                    ...
                    <span>{formatDate(paper.conference?.start_date)} - {formatDate(paper.conference?.end_date)}</span>
                 </div>
                 <div className="flex items-center text-slate-300">
