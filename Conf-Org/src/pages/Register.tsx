...
import { Mail, ArrowLeft, CheckCircle, Loader2, Lock, User, Building, Globe, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
const CAROUSEL_SLIDES = [
  {
    image: "https://nursingeducation.org/wp-content/uploads/2024/07/Research-Nurse-scaled-e1721688270245.jpg",
    title: "Join the Global Scientific Community",
    subtitle: "Connect with researchers and organizers worldwide."
  },
  {
    image: "https://online.maryville.edu/wp-content/uploads/sites/97/2023/09/social-researcher-1.jpg",
    title: "Streamline Your Event Management",
    subtitle: "From abstract submission to proceedings publication."
  },
  {
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTNEtCgkDqOCdEIYEUvjObtK6PD3rHfIQRxZw&s",
...
interface RegisterProps {
  ...
  onRegisterSuccess: (data: { name: string; email: string; role: string; roleId: number; avatar: string }) => void;
}

type RegStep = 'email' | 'sending' | 'verify' | 'profile' | 'success';

const Register: React.FC<RegisterProps> = ({ onNavigateHome, onNavigateLogin, onRegisterSuccess }) => {
  // --- Layout State ---
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userData && userData.length > 0) {
        ...
        if (!fetchError && roleData && roleData.user_roles && roleData.user_roles.length > 0) {
          ...
          <div className="p-6 sm:p-8 overflow-y-auto custom-scrollbar">
            ...
            {/* Steps Progress Bar */}
             {step !== 'success' && step !== 'sending' && (
               <div className="flex items-center gap-2 mb-8">
                 <div className={`h-1.5 flex-1 rounded-full transition-colors ${['email', 'sending', 'verify', 'profile'].includes(step) ? 'bg-brand-600' : 'bg-slate-100'}`} />
                 <div className={`h-1.5 flex-1 rounded-full transition-colors ${['verify', 'profile'].includes(step) ? 'bg-brand-600' : 'bg-slate-100'}`} />
                 <div className={`h-1.5 flex-1 rounded-full transition-colors ${step === 'profile' ? 'bg-brand-600' : 'bg-slate-100'}`} />
               </div>
             )}
            ...
            {step === 'email' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <Button onClick={handleEmailSubmit} className="w-full py-3 text-base shadow-lg shadow-brand-500/20" disabled={!email}>
                  Continue with Email
                </Button>
                ...
                <div className="text-center">
                  <p className="text-sm text-slate-500">
                    ...
                    <button onClick={onNavigateLogin} className="text-brand-600 hover:text-brand-700 font-semibold hover:underline">
                      Log in
                    </button>
                  </p>
                </div>
            ...
            {step === 'sending' && (
               <div className="flex flex-col items-center justify-center py-10 text-center animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <h3 className="text-xl font-semibold text-slate-900 mt-6">Sending Code...</h3>
            ...
            {step === 'verify' && (
              <div className="space-y-8 animate-in fade-in duration-300">
                <div className="text-center">
                  ...
                  <p className="text-slate-500 mt-2 text-sm">We sent a code to <span className="font-semibold text-slate-900">{email}</span></p>
                </div>
                ...
                <div className="space-y-3">
                  <Button onClick={handleVerifyCode} className="w-full py-3 shadow-lg shadow-brand-500/20" disabled={inputCode.length !== 6}>
                    Verify Email
                  </Button>
                  <button onClick={() => setStep('email')} className="w-full text-center text-sm text-slate-500 hover:text-slate-700 font-medium">
            ...
            {step === 'profile' && (
              <div className="animate-in fade-in duration-300">
                <div className="mb-6">
                  ...
                  <p className="text-slate-500 text-sm mt-1">Final step to setup your account.</p>
                </div>
                ...
                <form onSubmit={handleProfileSubmit} className="space-y-4">
                  ...
                  {/* Organization */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Organization (Optional)</label>
                    <div className="relative">
                  ...
                  <Button type="submit" className="w-full py-3 mt-2 shadow-lg shadow-brand-500/20" disabled={isSubmitting}>
            ...
            {step === 'success' && (
              <div className="text-center py-8 animate-in fade-in zoom-in-95 duration-500">
                 ...
                 <Button 
                    onClick={() => onRegisterSuccess({ name: fullName, email: email, role: assignedRole, roleId: assignedRoleId, avatar: '' })} 
